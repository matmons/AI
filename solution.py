...
class ASARProblem(object):
    ...
    def goal_test(self, state):
        ...
        if not state[2]:  # test if state Legs yet to be flown is empty
            ...
            return False
        for l in [[list(i[0])[0] + ' ' + list(i[0])[1]] + i[1:] for i in self.legs]:  # test if all Legs in »L« are in the state schedule »s[-1]«
            m = 0
            for cl in state[-1]:
                if l[0] not in cl[1]:
                    m = m
    ...
    def load(self, file):  # loads a problem from a file object f
        ...
        for line in open(file, 'r'):
            ...
            if line[0] == 'A':
                self.airports.append(line[1:])
            elif line[0] == 'C':
                self.aircraft_class.append(line[1:])
            elif line[0] == 'P':
                self.airplanes.append(line[1:])
            elif line[0] == 'L':
                self.legs.append([(line[1], line[2])] + line[3:])

        self.initial = [[[plane[0], None, None] for plane in self.airplanes], [0], self.legs, []]
        self.max_profit = 0
